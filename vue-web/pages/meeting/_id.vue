  ...
  <div>
    <div class="article-detail-container">
      <div class="back-button">
        <v-btn to="/meeting" text>Back</v-btn>
      </div>
      <div class="ar-title">
        {{ article.title }}
      </div>
      <div class="ar-time ar-options">
        Posted at {{ article.createdAt | timeFilter }}
        <br />
        参会人 : {{ article.persons.toString() }}
        <br />
        会议时间 : {{ article.startTime + '至' + article.endTime }}
      </div>
...
export default {
  ...
  async asyncData({ params, $axios }) {
    const { id } = params
    const { article } = await $axios.$get(`/meeting/${id}`)
    return {
      article
    }
  },
