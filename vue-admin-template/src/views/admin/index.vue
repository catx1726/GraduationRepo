    ...
    >
      <el-form ref="userEditForm" :model="userForm" :rules="userEditRules">
        ...
        <el-form-item label="用户名" :label-width="formLabelWidth" prop="name">
          <el-input v-model="userForm.name" autocomplete="off" placeholder="一个简单明了的登录名" />
        </el-form-item>
        <el-form-item label="密码" :label-width="formLabelWidth" prop="password">
          <el-input
            v-model="userForm.password"
            autocomplete="off"
            placeholder="随便你，总之要复杂一点"
          />
        </el-form-item>
        <el-form-item label="手机号" :label-width="formLabelWidth" prop="phone">
          <el-input v-model.number="userForm.phone" placeholder="18727698411" />
        </el-form-item>
        <el-form-item label="邮箱" :label-width="formLabelWidth" prop="email">
          <el-input v-model="userForm.email" autocomplete="off" placeholder="填不填都无所谓" />
        </el-form-item>
      </el-form>
...
<script>
import { adminList, editAdmin, deleteAdmin, addAdmin } from '@/api/admin'
...
export default {
  ...
  methods: {
    ...
    async searchMethod() {
      ...
      const res = await adminList(query)
    ...
    handleAddUser() {
      ...
      this.userForm = { phone: '', status: true, avatar: 'add-user-img-occupation', activitys: [] }
    ...
    async saveEdit() {
      ...
      if (check) {
        if (this.userForm._id) {
          ...
          // 新增
          const res = await addAdmin(this.userForm)
          console.log(res)
          if (res.status) {
            this.fetchUsers(this.query)
